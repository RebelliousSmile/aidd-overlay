...
When invoked, start with quick analysis using the **`/documentation-sync`** skill.

**Available modes:**
- `/documentation-sync sync [files]` - Sync code changes to documentation
- `/documentation-sync clean` - Clean up obsolete files (> 30 days)
- `/documentation-sync quick-ref <component>` - Create concise quick reference
- `/documentation-sync check` - Verify memory bank integrity
- `/documentation-sync optimize` - Audit and optimize token usage

```bash
# Quick memory bank check
